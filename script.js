...
function aggiungi_preferiti(event){
    ...
    if(tasto.textContent === "Aggiungi ai preferiti"){
        ...
        tasto.textContent="Già inserito nei preferiti";
    ...
    if(tasto.textContent === "Rimuovi dai preferiti"){
        const evidenziati=document.querySelectorAll("#città h2");

        for(let i=0; i<titoli.length; i++){
            if(evidenziati[i].textContent === titolo.textContent){
                const cambio=document.querySelectorAll("#città button");
...
function carica(){
    const tot_città=document.querySelectorAll(".scelte");
    for(let i=0; i<titoli.length; i++){
        ...
        tot_città[i].appendChild(titolo);
        tot_città[i].appendChild(immagine);
        tot_città[i].appendChild(descrizione);
        tot_città[i].appendChild(dettagli);
        tot_città[i].appendChild(aggiungi);

        descrizione.classList.add('hidden');
    }

    const contenuto=document.querySelectorAll("#città span");
    for(const cont of contenuto){
        cont.addEventListener("click",mostra);
    }

    const preferiti=document.querySelectorAll("#città button");
...
function cerca(event){
    ...
    const barra=document.querySelectorAll("#città h2");
